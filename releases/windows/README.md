...
```powershell
# Download the latest RC1 Windows asset from GitHub Releases, then:
$dest = Join-Path $env:USERPROFILE ".turbo\bin"
New-Item -ItemType Directory -Force -Path $dest | Out-Null
Copy-Item -Force .\turbo-1.0.0-rc.1-x86_64-pc-windows-msvc.exe (Join-Path $dest "turbo.exe")
turbo version
```
...
```powershell
$env:GROK_BINARY = (Resolve-Path .\turbo-1.0.0-rc.1-x86_64-pc-windows-msvc.exe).Path
```
...
```text
turbo version
# → turbo 1.0.0-rc.1 (<commit>)
```

Source tree `VERSION` may be newer (e.g. `1.0.0-rc.2` harness polish). Rebuild
with cargo for the latest wire; release assets are frozen ship builds only.
