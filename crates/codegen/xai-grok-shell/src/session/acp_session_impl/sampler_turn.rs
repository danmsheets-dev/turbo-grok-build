...
impl SessionActor {
    ...
    pub(super) async fn reconstruct_full_config(&self) -> SamplingConfig {
        ...
        SamplingConfig {
            ...
            // Resolved by the catalog-identity chain above (Codex / Kimi /
            // Claude / Radius / Copilot / xAI session), which subsumes the
            // plain `use_bearer_resolver` branch upstream inlines here.
            bearer_resolver,
    ...
    pub(crate) async fn handle_sampling_failure(
        ...
        {
            // Name the credential the server just rejected. Passing `None`
            // lets the early-out in `try_devbox_recovery` hand back whatever
            // `current()` holds — which here is the very bearer that produced
            // this 401 — and the caller then resubmits it until its retry
            // budget is gone. Same source as `auth::recovery`'s rejected id.
            let unusable = am.current().map(|a| a.key.clone());
            match am.try_devbox_recovery(unusable.as_deref()).await {
                Ok(auth) => {
                    tracing::info!(
                        session_id = %self.session_info.id.0,
