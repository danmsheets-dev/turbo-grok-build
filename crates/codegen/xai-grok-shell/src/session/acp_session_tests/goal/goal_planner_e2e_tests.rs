...
/// the planner exercises (Spawn â†’ result_tx). Returns the sender
...
/// active goal yet** â€” the caller drives `setup_goal` or
...
async fn planner_disabled_records_no_harness_trace_turn() {
    ...
    local
        .run_until(async {
            ...
            let turns = actor.chat_state_handle.take_harness_trace_turns().await;
            assert!(turns.is_empty(), "planner disabled â€” no trace turn");
        })
        .await;
}
...
async fn planner_success_sets_then_clears_planning_flag() {
    // The transient "planningâ€¦" badge fires before the subagent runs
...
async fn planner_clears_planning_latch_before_publishing_the_plan() {
    // Regression (Bugbot: "Late steering skips replan"): the "planningâ€¦" badge
    // must be cleared at the commit-to-publish point â€” the moment the planner
    // run is taken and we commit to publishing the produced plan â€” NOT only at
    ...
    local
        .run_until(async {
            ...
            // sequence is exactly the transient badge on then off â€” no duplicate
...
async fn planner_fail_closed_clears_planning_flag() {
    ...
    // "planningâ€¦" badge can never stick on screen.
    let local = tokio::task::LocalSet::new();
    local
        .run_until(async {
...
async fn planning_badge_survives_intervening_goal_update() {
    ...
    // that fires while the planner runs must NOT clear the "planningâ€¦"
...
async fn reconcile_is_idempotent_via_atomic_flag() {
    ...
    local
        .run_until(async {
            ...
            // atomic short-circuit must skip the work â€” a regression
...
async fn planner_subagent_tokens_fold_into_goal_total() {
    ...
    local
        .run_until(async {
            ...
            // (anchor=0 â†’ last=12_000). The handler tags the record
...
/// Lifecycle regression guard: planner fails â†’ goal paused with
/// canonical message â†’ `GoalResume` re-fires the planner â†’ second
/// attempt succeeds â†’ goal Active with `plan_file` set.
/// Pins the retry-on-resume contract â€” without it, the pause
...
/// Repeated-failure variant: planner fails â†’ resume â†’ planner
/// fails again â†’ goal re-paused with the canonical message. Pins
...
async fn lifecycle_fail_pause_resume_retry_fail_repauses() {
    ...
    local
        .run_until(async {
            ...
            // Planner re-failed â†’ goal re-paused â†’ resume must end the turn
...
/// re-fire the planner (defensive â€” the retry path keys off
/// `plan_file.is_none()`).
#[tokio::test(flavor = "current_thread")]
#[serial]
...
/// no-plan block â€” no dangling `Plan:` pointer, no plan-aware phrasing â€”
