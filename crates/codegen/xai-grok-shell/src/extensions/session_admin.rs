...
pub(crate) async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    match args.method.as_ref() {
        ...
        // Turbo-only internal methods: these have no `InternalMethod` variant
        // (they are never fanned out over the leader wire), so `from_name`
        // returns `None` above and they fall through to this string match.
