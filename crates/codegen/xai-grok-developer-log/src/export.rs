//! Export packs for the Hyper Development Team.
...
pub fn export_pack(
    ...
    write_fingerprints_csv(&csv_path, &entries)?;

    let summary_path = out_dir.join("summary.md");
    fs::write(&summary_path, render_summary_md(&incidents, &entries))?;

    let manifest = serde_json::json!({
        "schema_version": crate::schema::SCHEMA_VERSION,
        "exported_at": Utc::now().to_rfc3339(),
        "hyper_version": xai_grok_version::installed(),
...
fn render_summary_md(incidents: &[Incident], entries: &[IndexEntry]) -> String {
    let mut p0 = 0u32;
    let mut p1 = 0u32;
    let mut p2 = 0u32;
    let mut p3 = 0u32;
    ...
    out.push_str("# Hyper Auto Developer Log — Export Summary\n\n");
    out.push_str(&format!(
        "- **Exported at:** {}\n",
        Utc::now().to_rfc3339()
    ));
    ...
    out.push_str(
        "\n---\n*Generated by Hyper Auto Developer Log. Fields are redacted for secrets and user paths.*\n",
    );
    out
}
