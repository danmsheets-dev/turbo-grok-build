...
fn render_footer(
    ...
    // focused, else the second-`Ctrl+X` "press again" hint. Only paint it
    // while the confirm window is actually live — the dispatcher re-arms
    // (rather than deletes) on a press after [`super::state::CONFIRM_WINDOW`],
    // so an expired arm must not keep claiming the footer (e.g. after a
    // mouse click moved the selection without a keypress to disarm it);
    // `armed_delete_row_ref` applies that expiry for us.
    if state.armed_delete_row_ref().is_some() {
        if state.list_focused {
            let hints = vec![
                HintItem::new(
                    key!('y'),
                    crate::i18n::tr_or("hints.confirm_delete", "confirm delete"),
                ),
            ...
            let pending = PendingHint {
                shortcut: stop_key,
                label: crate::i18n::tr_or("hints.delete_session", "delete this session"),
            };
            ShortcutsBar::new(&[])
                .with_pending(Some(pending))
                .render(inner, buf);
    ...
    let stop_label = if matches!(
        ...
        crate::i18n::tr_or("hints.delete", "delete")
    };
    ...
    let hints: Vec<HintItem> = if peek_active {
        ...
        // populated rather than silently empty. No stop/delete chip
        // here: with nothing selected it would have no row to act on.
        vec![HintItem::new(send_key, rust_i18n::t!("hints.create"))]
    };
