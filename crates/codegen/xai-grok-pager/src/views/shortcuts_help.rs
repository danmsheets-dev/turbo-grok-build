...
/// Bundle key for a pseudo-row's long-help const (paste/undo/redo/history/
...
fn pseudo_long_help_key(lh: &str) -> &'static str {
    ...
    } else if lh == HISTORY_LONG_HELP {
        "shortcuts_help.history_long_help"
    } else if lh == SCROLLBACK_SEARCH_LONG_HELP {
        "shortcuts_help.scrollback_search_long_help"
    } else {
        ""
    }
}
