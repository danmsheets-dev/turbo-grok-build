...
pub struct PagerArgs {
    ...
    /// Not supported in Turbo: upstream gates this on its
    /// `streaming-messages-json` reducer output, which Turbo does not carry.
    /// Accepted so the flag errors clearly rather than being an unknown
    /// argument; use `--output-format streaming-json` instead.
    ...
    pub no_leader: bool,
    /// Initial prompt for the interactive session, e.g. `turbo "fix the bug"` or `turbo --worktree=feat "create this feature"`.
    #[arg(
        value_name = "PROMPT",
        conflicts_with_all = &["single",
        "prompt_json",
...
impl PagerArgs {
    ...
    pub fn parse_cli() -> Self {
        let bin_name = std::env::args()
            ...
            .and_then(|n| n.to_str())
            .filter(|n| *n == "turbo" || *n == "grok" || *n == "agent")
            .unwrap_or("turbo")
            .to_owned();
    ...
    /// whitespace. This is the `turbo "<prompt>"` launch form; the headless
