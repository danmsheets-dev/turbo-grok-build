//! Browser-verification policy used by plan/UI work when `browser_*` tools exist.
//!
//! The system-prompt `<browser_verification>` block is gated by
//! [`crate::prompt::context::PromptContext::include_browser_verification`].
//! This module keeps the longer first-user-message wording in one place so a
//! later user-message merge can reuse it without forking the policy.
...
mod tests {
    ...
    #[test]
    fn synthetic_rule_is_nonempty_and_mentions_browser() {
        let rules = synthetic_user_rules();
        assert_eq!(rules.len(), 1);
        assert!(rules[0].path.is_empty());
        assert!(USER_RULE.contains("verify your work in the browser"));
        assert!(USER_RULE.contains("Hunt for regressions"));
    }
}
