...
pub use super::coordinator_state::{
    ...
    CompletionDisposition, CoordinatorConfig, DEFAULT_MAX_CONCURRENT_SUBAGENTS, LimitedSpawnOrigin,
    LocalBoxFuture, MAX_COMPLETED_ENTRIES, SendBoxFuture, StartedChild, SubagentLimitDecision,
    SubagentLimitNotice, SubagentLimitSink, SubagentProgress,
};
...
impl<R: ChildRunner> SubagentCoordinator<R> {
    ...
    fn cancel_loop_task_children(&mut self, loop_task_id: &str, parent_session_id: Option<&str>) {
        ...
        let queue_dropped = self.remove_queued(|request| {
            request.runtime_overrides.loop_task_id.as_deref() == Some(loop_task_id)
                && belongs_to_session(request, parent_session_id)
        });
        cancelled += queue_dropped as u32;
        if cancelled > 0 {
            tracing::info!(
                loop_task_id,
                cancelled,
