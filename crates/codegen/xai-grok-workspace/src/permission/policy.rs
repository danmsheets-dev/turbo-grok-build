...
/// Match Read/Edit/Grep against a path-mode rule using **every** spelling that
/// denotes the same operand. Two layers, unioned (a match on either is a match,
/// so a deny can never be dodged by picking a different spelling):
///
/// 1. Lexical: normalize (+ cwd-join) and try the absolute form plus the
///    cwd-relative / `./`-prefixed forms ([`path_match_forms`]). Rooted patterns
///    are self-containing — `..` never survives normalization and relative
///    spellings are minted only for paths genuinely under the cwd — so
///    `Read(./**)` / `Read(src/**)` cannot be escaped via traversal. Unrooted
///    patterns (`*`, leading `**`) keep their any-depth meaning.
/// 2. Canonical: resolve the operand (and the rule's literal path prefix)
///    through confine's `canonicalize_for_permission` ([`path_glob_matches`]),
///    so short-name (`MAINRE~1`), symlink and case aliases of a denied
...
fn path_context_matches(path: &str, cr: &CompiledRule<'_>, cwd: Option<&Path>) -> bool {
    ...
    // canonicalising layer so a cwd-joined relative operand is covered too.
    path_match_forms(path, cwd).iter().any(|text| {
        glob_matches(text, MatchContext::Path, cr.matcher)
            || path_glob_matches(text, pattern, cr.matcher)
    })
}
