...
"#,
    ...
    // Bound every wait — a hung evaluator used to freeze session load.
    ...
    let output = match run_with_deadline(bash_cmd, deadline, "bash") {
        ...
        RunOutcome::Failed => {
            tracing::warn!(?envrc_path, bash = %bash, "Failed to run bash for .envrc");
            return None;
        }
    };
...
mod tests {
    ...
    fn test_simple_export() {
        ...
        let Some(env) = load_envrc_with_timeout(dir.path(), Duration::from_secs(10)) else {
